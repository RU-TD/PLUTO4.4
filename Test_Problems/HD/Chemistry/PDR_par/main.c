 ...
 *********************************************************************** */
{
  ...
  /* Chemistry module */
  #if CHEMISTRY != NO
   double crflux;
   int cooling_mode, heating_mode;
   crflux = 5e-17;  // cosmic rays ionization rate, 1/s
   for (nv=0;nv<NTRACER;nv++) x[nv] = 0.e0;
   for (nv=0;nv<NPHOTO;nv++) jflux[nv] = 0.e0;

   // use only CR heating and SML97 cooling
   cooling_mode = 0;
   heating_mode = 0;
   ...
   // set thermal mode variables
   prizmo_set_cooling_mode_c(&cooling_mode);
   prizmo_set_heating_mode_c(&heating_mode);
   ...
   if (prank == 0) {
     ...
     for (k = KBEG; k <= KEND; k++)
     {
       for (j = JBEG; j <= JEND; j++)
       {
         ...
         prizmo_get_draine_flux_c(irradiation.jflux[k][j][IBEG]);
         // loop to multiply 2*pi*G0
         for(int l = 0; l < NPHOTO; l++){
           irradiation.jflux[k][j][IBEG][l] *= Gnot * 2e0 * pi;
           print("SZIIIIIIA %le\n",irradiation.jflux[k][j][IBEG][0]);
         }
       }
     }
   }
